...
In older AVR-GCC versions, variables tagged with the \lstinline{PROGMEM} attribute don't have to also be marked as \lstinline{const}, however this is changing in newer releases. To prevent compile time warnings, we add this to the variable's declaration:
...
Now that the \lstinline{PROGMEM} string has a const" modifier, we cannot try to modify it in our code. The pgmspace.h header also exposes a neat little macro, \lstinline{PSTR()}, which by some GCC magic allows you to create inline strings:
